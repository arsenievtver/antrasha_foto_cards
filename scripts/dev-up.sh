#!/usr/bin/env bash
# Запуск Postgres (Docker), миграции, API, Vite: client, admin, work, xfashion, giftcard.
set -euo pipefail
...
if [[ "${SKIP_GIFTCARD:-0}" == "1" ]]; then
	echo "[up] SKIP_GIFTCARD=1 — сертификаты не поднимаю"
elif [[ ! -d "$REPO_ROOT/giftcard/node_modules" ]]; then
	echo "[up] сертификаты пропущены: нет giftcard/node_modules — npm install --prefix giftcard"
elif [[ -f "$PID_GIFTCARD" ]] && kill -0 "$(cat "$PID_GIFTCARD")" 2>/dev/null; then
	echo "[up] сертификаты уже запущены (PID $(cat "$PID_GIFTCARD")) — пропуск"
else
	(
	...
	echo "[up] сертификаты http://127.0.0.1:${GIFTCARD_PORT} (лог: $LOG_GIFTCARD, PID: $PID_GIFTCARD)"
fi
...
while read -r lan_ip; do
	...
	echo "              сертификаты http://${lan_ip}:${GIFTCARD_PORT}"
...
echo "(Firewall: TCP ${FRONTEND_PORT}, ${ADMIN_PORT}, ${WORK_PORT}, ${XFASHION_PORT}, ${GIFTCARD_PORT}; API — ${BACKEND_PORT})"
echo "(Только Xfashion: npm run dev:xfashion:up / dev:xfashion:down)"
echo "(Только сертификаты: npm run dev:giftcard:up / dev:giftcard:down)"
echo "──────────────────────────────────────────────────────────────"
