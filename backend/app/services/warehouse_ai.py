...
- **Сезон** в имени/модификации: маркеры **`ВЛYYYY`** / **`ОЗYYYY`**, либо дата коллекции в артикуле **`/MM.YY`** (напр. `/02.26` = ВЛ2026).
  Календарные окна продаж по умолчанию:
...
1. Найти поставщика (`counterparty_list` search=бренд) — один вызов.
2. `report_profit_byproduct` за период с **`filter=supplier={href}`** (+ store при необходимости). Это отчёт «Прибыльность», как в UI.
3. Если нужен сезон коллекции — оставить строки с маркером ВЛ/ОЗ или датой `/MM.YY` в артикуле. Не выгружать весь каталог из 600+ товаров ради матча id.
4. Не обходить productfolder_* ради бренда. Discovery ≤ 2–3 вызова.
